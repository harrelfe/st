...
date: 2018-03-26
modified: 2018-04-04
categories: [RCT, bayes, design, drug-evaluation, evidence, hypothesis-testing, medicine, multiplicity, p-value, posterior, endpoints, 2018]
...
* binary endpoints [are not statistically efficient anyway](../ordinal-info)
...
One can readily see that once you get away from null hypothesis testing, many clinically relevant possibilities exist, and multiplicity considerations are cast aside.  A reasonable strategy would be to demand an extra-high probability of hitting any one of three targets, or a somewhat lower probability of hitting any two of the three targets.  More about this way of thinking may be found [here](../bayes-freq-stmts)[^8].
...
[^1]: Disclosure: I led the independent statistical team at
...
**Christopher Harvey**: Thanks for summarizing a Bayesian perspective on multiple endpoints. Footnote 8 is clear. But if many predictive probabilities are computed and only the few interesting ones emphasized, how externally valid are these emphasized probabilities? In the section prior to the footnote, I am assuming the “extra high” probability demanded of hitting 1 of m targets will be higher with higher m. Is this concept not similar to multiplicity? Will have to check out that link.

**Frank Harrell**: Thanks for raising these issues. I think the key issue is that the probability of a union of events must be >= the highest probability of one of the events. So it's a necessity that a more inclusion probability must be >= a simple probabilities. So having minor events' evidence quantified does not influence the evidence base for events of major emphasis. The concept of demanding a higher posterior probability of an effect for a less important outcome has little to do with multiplicity and everything to do with rules of evidence. Think about various cases the police are working on. Before arresting someone for murder, the probability that the suspect committed the crime must be greater than the probability someone else stole a loaf of bread.
